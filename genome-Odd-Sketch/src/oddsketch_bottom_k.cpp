...
int main(){
    ...
    const double J_threshold = 0.75; // これより小さい場合誤差が大きくなるため、J＝０とする
    const uint32_t numHashFunc = sketch_size; // n/{4(1-J_threshold)}=n,since J_threshold=0.75
    ...
    for(int i=0; i<input_num; i++){
        int t;
        std::cin>>t;
        input_lens.push_back(t);

    }
    std::cout<<input_lens[0]<<std::endl;
