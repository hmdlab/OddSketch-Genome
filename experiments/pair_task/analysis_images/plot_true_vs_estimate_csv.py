...
実行例:
  cd experiments/pair_task/analysis_images && python plot_comparison_from_csv.py \
      --csv ../data/test_genomes/comparison_results.csv
  別推定列を指定（BinDashなど）:
      python plot_comparison_from_csv.py --est-col jaccard_bindash \
...
def main():
    ...
    ap.add_argument('--csv', default=str(Path(__file__).resolve().parent.parent / 'data' / 'test_genomes' / 'comparison_results.csv'))
