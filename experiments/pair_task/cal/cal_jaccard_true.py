...
def main():
    ...
    if os.path.isabs(cfg_arg):
        cfg_candidates_cli.append(cfg_arg)
    else:
        cfg_candidates_cli.append(os.path.join(script_dir, "..", cfg_arg))
        cfg_candidates_cli.append(os.path.join(os.getcwd(), cfg_arg))

    # まず C++ 実装があればそれを呼び出して高速に計算
    try:
        import subprocess
        repo_root = os.path.abspath(os.path.join(script_dir, '..', '..', '..'))
        cpp_bin = os.path.join(repo_root, 'src', 'cal', 'true_jaccard')
        if os.path.exists(cpp_bin) and os.access(cpp_bin, os.X_OK):
            ...
            cfg_path = next((c for c in cfg_candidates_cli if os.path.exists(c)), os.path.join(script_dir, '..', 'pipeline_config.json'))
            cmd = [
                cpp_bin,
                f"--config={cfg_path}",
    ...
    cfg_path_candidates = cfg_candidates_cli + [
