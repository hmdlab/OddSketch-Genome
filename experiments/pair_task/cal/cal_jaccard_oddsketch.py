...
def resolve_oddsketch_bin() -> str:
    """Locate oddsketch binary across common layouts.

    Priority:
    1) ODDSKETCH_BIN env var
    2) repository-root/src/oddsketch
    3) legacy relative path ../oddsketch
    """
...
def run_oddsketch_sketch(genome_files, cfg):
    ...
    pos_mode = oddcfg.get('pos_mode', 'mix')  # mix|stripe
    ...
    try:
        ...
        cmd = [resolve_oddsketch_bin(), 'sketch', f'--kmer={kmer}', f'--sketch-size={ssize}', f'--j0={j0}', f'--pos-mode={pos_mode}']
...
def run_oddsketch_dist(sketch_files, cfg):
    ...
    try:
        ...
        cmd = [resolve_oddsketch_bin(), 'dist', f'--kmer={kmer}', f'--sketch-size={ssize}', f'--j0={j0}', f'--pos-mode={pos_mode}']
        result = subprocess.run(
            cmd,
