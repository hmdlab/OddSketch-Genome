...
## BinDash sketch 実行
同じ `paths.local_genome_list` の RefSeq genome に対して BinDash の一括 sketch 時間を測る場合は、root ディレクトリから次を投入します。
...
BinDash 用の設定は `config.json` の `bindash` セクションで指定します。`bindash.sketch_size` は target bit 数として扱い、runner が `--sketchsize64` に変換します。既定では OddSketch と同じ `threads=8`, `kmerlen=64`, `sketch_size=16384` です。

出力は OddSketch と同じく `data/sketch_runs/runs/<run_id>/` 配下に保存します。

- `metadata/used_config.json`
- `metadata/run_metadata.json`
- `manifests/bindash_genome_paths.txt`
- `manifests/bindash_sketch_files.tsv`
- `bindash_sketches/bindash_refseq_sketch*`
- `results/bindash_sketch_metrics.tsv`
- `logs/bindash_sketch_stdout.txt`
- `logs/bindash_sketch_time.txt`

`bindash_sketch_metrics.tsv` の `elapsed_sec` は BinDash 本体の sketch 実行時間、`workflow_elapsed_sec` は runner 側の準備を含む時間です。
